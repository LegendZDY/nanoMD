...
def random_forest_data(samFile,base,fout):
    ...
    """
    transcript_lengths = {}
    sam = linecache.getlines(samFile)
    S = re.compile(r'\d+S')
    if base == "U":
        T = re.compile(r'T|t')
    elif base == "A":
    ...
    with open(fout,"w") as ft:
        if base == "U":
            ft.write("read,UmisRate,UmisQrate,t_a_rate,t_a_Qrate,t_c_rate,t_c_Qrate,t_g_rate,t_g_Qrate,del_rate\n")
        elif base == "A":
            ft.write("read,AmisRate,AmisQrate,a_t_rate,a_t_Qrate,a_c_rate,a_c_Qrate,a_g_rate,a_g_Qrate,del_rate\n")
        elif base == "G":
            ft.write("read,GmisRate,GmisQrate,g_a_rate,g_a_Qrate,g_c_rate,g_c_Qrate,g_t_rate,g_t_Qrate,del_rate\n")
        elif base == "C":
            ft.write("read,CmisRate,CmisQrate,c_a_rate,c_a_Qrate,c_t_rate,c_t_Qrate,c_g_rate,c_g_Qrate,del_rate\n")
        else:
        ...
        for line in sam:
            ...
            if line.split()[1] == "0" or line.split()[1] == "16":
                cs = line[cstr.search(line).span()[0] + 5:cstr.search(line).span()[1]]
                read, transcript, cigar, quality, sequence, Q = line.split()[0], line.split()[2], line.split()[5], int(line.split()[4]), line.split()[9], line.split()[10]
                ...
                if S.match(cigar) != None:
                    numS = int(cigar[S.match(cigar).span()[0]:S.match(cigar).span()[1] - 1])
                    readStart = numS
                while len(cs) != 0:
                    if identical_block.match(cs) != None:
                        idenum = int(cs[identical_block.match(cs).span()[0]+1:identical_block.match(cs).span()[1]])
                        if len(matchseq) == 0:
                            matchseq += str(sequence[readStart:idenum])
                        else:
                            matchseq += str(sequence[readStart:idenum+readStart])
                        readStart += idenum
                        cs = cs[identical_block.match(cs).span()[1]:]
                    elif deltion_block.match(cs) != None:
                        delStr = cs[deltion_block.match(cs).span()[0]+1:deltion_block.match(cs).span()[1]]
                        ...
                        readStart += len(cs[insertion_block.match(cs).span()[0]+1:insertion_block.match(cs).span()[1]])
                        cs = cs[insertion_block.match(cs).span()[1]:]
                    elif substituted_ref_read.match(cs) != None:
                        refStr = cs[substituted_ref_read.match(cs).span()[0]+1:substituted_ref_read.match(cs).span()[1]][0]
                        readStr = cs[substituted_ref_read.match(cs).span()[0]+1:substituted_ref_read.match(cs).span()[1]][1]
                        cs = cs[substituted_ref_read.match(cs).span()[1]:]
                        if refStr == "t" and base == "U":
                            accuracy = 1 - 0.1 ** ((ord(Q[readStart-1]) - 33) / 10)
                            UmisNum += 1
                            UmisQ += accuracy
                            if readStr == "a":
                                t_a += 1
                ...
                read_mean_Q = sum([1 - 0.1 ** ((ord(Q[i]) - 33) / 10) for i in range(len(Q))])/len(Q)
                if UmisNum != 0:
                ...
                ft.write("{},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}\n".format(read,UmisRate,UmisQrate,t_a_rate,t_a_Qrate,t_c_rate,t_c_Qrate,t_g_rate,t_g_Qrate,del_rate))
def main():
